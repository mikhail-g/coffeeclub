...
- **`notion-update-page` cannot set multiple values on a multi-select property — use `curl` with the Notion REST API instead.** The MCP only accepts `string | number | null` per property, so it always treats the value as a single option name. Passing `"Filter, Espresso, Omni"` (any separator) fails with a validation error. This is a known bug in the Notion MCP server: [makenotion/notion-mcp-server#108](https://github.com/makenotion/notion-mcp-server/issues/108). The fix: call the Notion REST API directly. A reusable script is at `.claude/skills/sync-roaster/scripts/set-multi-select.sh`:
  ```bash
  .claude/skills/sync-roaster/scripts/set-multi-select.sh "<page-id-dashed>" "Field" Value1 Value2
  ```
  The script uses `NOTION_ACCESS_KEY` from `.env` in the project root. Under the hood it calls:
  ```bash
  curl -s -X PATCH "https://api.notion.com/v1/pages/<page-id-dashed>" \
    -H "Authorization: Bearer $NOTION_ACCESS_KEY" \
    -H "Notion-Version: 2022-06-28" \
    -H "Content-Type: application/json" \
    -d '{"properties": {"Field": {"multi_select": [{"name": "A"}, {"name": "B"}]}}}'
  ```
